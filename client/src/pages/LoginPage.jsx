  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/auth/login', {
        ...
        credentials: 'include', // enables cookie handling
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#000000] px-4">
      {loading ? (
        <div className="flex flex-col items-center">
          <svg className="animate-spin h-8 w-8 text-teal-200 mb-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
        ...
        <div className="bg-black p-6 sm:p-8 rounded-xl shadow-xl w-full max-w-sm sm:max-w-md">
          <h2 className="text-center text-xl sm:text-2xl font-bold text-teal-400 mb-2">Welcome Back!</h2>
          <p className="text-center text-sm text-white mb-6">
            Let's login to continue tracking your sugar levels.
          </p>

          {error && <div className="text-center text-sm text-red-500 mb-4 font-medium">{error}</div>}
